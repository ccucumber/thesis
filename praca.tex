...
\subsection{Detekcja magnetorotacji}

Światło dostrojone do przejścia atomowego powoduje powstanie pewnego rodzaju uporządkowania w populacji atomów.
Proces ten nazywany jest pompowaniem optycznym. Fotony, mając określony moment pędu, mogą oddziaływać tylko z atomami w określonym stanie, który także można opisać poprzez orientację momentu pędu atomu w przestrzeni.

//CO NIECO O STANACH m, MoŻE RYSUNECZEK "POMPOWANIE OPTYCZNE", może alignmenty?//

Ścisły opis pompowania optycznego wykorzystuje macierz gęstości i jej równanie ewolucji - r.  Liouville'a -- von Neumanna, w niniejszej pracy skupię się jednak na omawianiu obserwowalnych efektów, a nie teorii jaka za nimi stoi.

Populacja atomów, z wytworzoną orientacją momentów magnetycznych, wykazuje inny współczynnik absorpcji dla poszczególnych polaryzacji światła. Orientacja wytworzona przez światło o polaryzacji kołowej prawoskrętnej będzie słabo absorbować polaryzację prawoskrętną (optyczne ,,napompowanie") i bardzo silnie polaryzację ortogonalną - prawoskrętną. Skoro absorpcje są różne, również rzeczywiste części współczynników załamania muszą się różnić między polaryzacjami -- występuje anizotropia optyczna ośrodka. Zjawisko to dla polaryzacji liniowej zachodzi dokładnie tak samo, choć nieco różni się w opisie teoretycznym.

Kluczowym dla możliwości pomiaru pola magnetycznego jest fakt, że momenty magnetyczne atomów precesują w polu magnetycznym -- jest to precesja Larmora. Precesja ta jest tożsama z obrotem wszystkich obserwabli w przestrzeni.
Umieszczenie populacji atomów napompowanych światłem spolaryzowanym liniowo w słabym polu magnetycznym sprawi więc, że wyróżniona przez atomy płaszczyzna polaryzacji światła zacznie się obracać względem kierunku pola magnetycznego, z częstością Larmora.

Powstanie powyższej płaszczyzny jest jednak efektem pompowania światłem, którego płaszczyzna polaryzacji jest nieruchoma. W miarę jak orientacja momentów magnetycznych ,,próbuje" się obrócić w skutek precesji Larmora, światło pompujące ,,na bieżąco" przywraca orientacje atomów zgodną z płaszczyzną polaryzacji światła,
w skutek czego orientacja atomów nie obraca się w przestrzeni, tym niemniej jest ,,przekręcona" względem pierwotnego położenia.
Polaryzacja światła liniowo spolaryzowanego, o p pol niewspółliniowej z orientacją atomów ulega skręceniu w stronę, w którą zachodzi precesja Larmora. Zmiana kierunku pola magnetycznego powoduje więc zmianę kierunku skręcenia na przeciwną.

Opisane zjawisko pozwala jedynie precyzyjnie wyznaczyć zero pola magnetycznego -- skręcenie płaszczyzny polaryzacji światła jest wtedy zerowe.

//Może obrazek zeropolowy?// 
 
Aby móc zmierzyć wartość pola magnetycznego, opracowana została następująca technika: (CYTOWANIE?)
Światło, również liniowo spolaryzowane, jest modulowane w czasie. Modulacja może dotyczyć mocy światła (technika AMOR), ale i częstotliwości (długości fali światła) (FMNMOR).

Modulacja sprawia, że cyklicznie, przez jedną część czasu atomy nie oddziałują ze światłem (z powodu jego wygaszenia lub odstrojenia od przejścia) i precesują swobodnie, a przez drugą są pompowane. Jeśli częstotliwość modulacji nie pokrywa się z częstością Larmora, pompowanie następuje przypadkowo i nie powoduje zorientowania momentów magnetycznych atomów. Jeśli jednak częstość modulacji jest równa podwojonej częstotliwości Larmora (lub, w ogólności jej stosunek do częstości Larmora jest liczbą wymierną), światło zawsze natrafia na atomy w tej samej fazie precesji i buduje orientację momentów magnetycznych.

Metoda pozwala więc precyzyjnie wykrywać, czy pole magnetyczne, a dokładnie wyznaczana przez nie częstość Larmora ma wartość większą niż zadana przez częstotliwość modulacji (zachodzi wtedy skręcenie polaryzacji w kierunku precesji Larmora), czy też mniejszą (polaryzacja skręca się wtedy w kierunku przeciwnym).


\subsection{Stabilizacja długości fali - pomiar fazoczuły}

Skręcenie polaryzacji z powyższego punktu zachodzi synchronicznie do częstotliwości modulacji. Można to zwizualizować w następujący sposób: orientacja atomów po okresowym włączeniu światła (dostrojeniu do przejścia) jest ,,przed" kierunkiem polaryzacji światła, i obracając się, finalnie ,,wyprzedza" płaszczyznę polaryzacji światła, po czym światło jest wyłączane. 

Warto więc zastosować do skręcenia detekcję synchroniczną.
 

Korzystając z tego, że długość fali lasera jest modulowana, ze światła przechodzącego przez komórkę można uzyskać jeszcze jedną użyteczną informację -- aktualne odstrojenie długości fali światła od linii absorpcyjnej. Jest to parametr konieczny, by móc uruchomić automatyczne dostrajanie lasera do przejścia atomowego.


\begin{figure}[h!]
\centering
 \includegraphics[width=\textwidth]{lock_g5.eps}
 % front_all.svg: 1153x455 pixel, 72dpi, 40.68x16.05 cm, bb=0 0 1153 455
 \caption{Zasada działania //CZEGO?//}
 \label{fig:spekpara}
\end{figure}
...
Rysunek \ref{panodp} przedstawia zależność sygnału błędu od mocy światła przechodzącego przez komórkę. Poszerzenie sygnału ze wzrostem mocy światła praktycznie nie występuje. Oznacza to, że moc światła nie jest zbyt wysoka, tj. nie wysyca populacji atomów w komórce.
...
c) Amplituda rezonansu na 2  w zależności od $P_{las}$ d) Szerokość rezonansu na 2 harmonicznej w zależności od $P_{las}$}
\label{wykresikiodP}
\end{figure}
